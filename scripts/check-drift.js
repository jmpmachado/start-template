...
const CONFIGURE_EXCLUDE = [
  ...
  'scripts/init-agile.js',                            // wizard that writes [CONFIGURE] strings
  'scripts/lib/agile-config.js',                      // wizard lib — replaces [CONFIGURE] strings
  '.agent/context/AGILE_GUIDE.md',                    // template doc — [CONFIGURE] is instructional
  '.agent/context/TEMPLATE_UPGRADE_PROTOCOL.md',      // this protocol — references [CONFIGURE] by name
  ...
  '.agent/context/PEER_REVIEW_TEMPLATE.md',           // references [CONFIGURE] as a token name in checklist item 14, not an unfilled field
  '.agent/context/PRIVACY_CONTACTS.md',               // adoption-time template — [CONFIGURE] fields are intentional until project adopts GDPR/LGPD contact details
  '.agent/context/PRIVACY_NOTICE_TEMPLATE.md',        // adoption-time template — [CONFIGURE] fields filled by legal counsel at go-live, not by the template itself
  ...
  '.agent/context/CODEX_MIN_01_SELF_CONTAINED.md',   // template-process handoff — [CONFIGURE] placeholders are intentional sprint-setup fields, removed by wizard (TEMPLATE_ONLY_FILES)
  '.agent/context/HANDOFF_CODEX_MIN_01.md',          // template-process handoff — references [CONFIGURE] as context for adoption sprint
  '.agent/context/HANDOFF_GEMINI_MIN_02.md',         // template-process handoff — same rationale
];
...
function checkAgileConfigFilled() {
  ...
  if (count >= 10) {
    ...
    crack('medium', '.agent/context/AGILE_CONFIG.md', `contains ${count} unfilled [CONFIGURE] placeholder(s) — run "npm run init-agile" or fill manually`);
  }
}
